...
class OSDEE:
    def __init__(self, net: int | pp.pandapowerNet) -> None:
        ...
        self._prim = _prim(
            self,
            int(self._param["initial_weight_prim"]),
            self._param["attribute_weight_prim"],
        )
        self._ms = _ms(
            self,
            float(self._param["variacao_ms"]) / 100,
            int(self._param["quantidade_csq"]) // 2,
        )
        self._vns = _vns(self)
    ...
    @staticmethod
    def set_net_from_graph(
        net: pp.pandapowerNet, graph: nx.MultiGraph
    ) -> pp.pandapowerNet:
    ...
    def _set_net_from_id(
        self, net: pp.pandapowerNet, id: tuple[int]
    ) -> pp.pandapowerNet:
        qtd_gd = id[0]
        id = id[1:]
        bus_gd = id[:qtd_gd]
    ...
    def set_gd_in_buses(self, net) -> pp.pandapowerNet:
        ...
        for _ in range(self._qtd_gd):
            for min_pu_bus in net.res_bus.vm_pu.sort_values().index:
                try:
                    net.gen.loc[net.gen.bus == min_pu_bus, "in_service"] = True
    ...
    def run_vnd_in_ms_systems(self, net, ms_group: set[tuple[int]]):
        ...
        for ms_instance in ms_group:
            ...
            if self._multi_vnd:
                net = self.vns.runmulti(net, OSDEE.get_graph_from_net(net))
            else:
    ...
    def _get_results_path(self, net: pp.pandapowerNet, minLoss: float) -> str:
        ...
        path += (
            "iwp"
            + self._param["initial_weight_prim"]
            + "_msv"
            + self._param["quantidade_csq"]
            + "_gdq"
